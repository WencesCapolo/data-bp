...
`100011`/`100030` → Mensual Total). The Cobros Export omits that column, so every
uploaded Cobro would otherwise land in `Otros` and destroy the subscription-mix
...
back to the book when it is null. A Cobro at a price the book does not contain
...
510k existing Cobros carry a real `price_id`, the model could be scored against
...
Total in 14 997. Uploads only ever carry recent Cobros, so an exact book of current
prices is both sufficient and checkable.

## Consequences

- The book was mined from labelled rows, not guessed: monthly successful Cobros
