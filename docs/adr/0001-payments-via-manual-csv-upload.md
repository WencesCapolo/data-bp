...
# Cobros enter the mirror by manual CSV upload, not by API

The Platform endpoint that served the Cobros ledger (`/intel/export/payments`) stopped
...
So a person downloads the Cobros Export from the Control Panel and uploads it through
...
- A complete Sync now requires a human in the loop. The 6-hourly `SyncScheduler` keeps
  refreshing everything except Cobros, so dashboards never go fully stale, but Cobros
...
- This is expected to be temporary. Reading Cobros directly from MercadoPago and Stripe
