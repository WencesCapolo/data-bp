...
and share identical columns: the **Cobros Export** (a ledger, includes failed
attempts) and the **Suscripciones Export** (a snapshot of currently-active subs).
_Avoid_: report, dump, download

**Upload**:
A Cobros Export handed to this app by a person through the UI, replacing the
...
**Subscription**:
A Subscriber's ongoing right to watch, established by a successful Cobro and
...
**Period**:
How many days of access one Cobro grants — 30, 365, or 0 for access that carries
no recurring right.
...
Every Cobro belongs to exactly one, and a Cobro whose Subscriber is unknown to
...
Window determines which Cobros it can possibly contain.
