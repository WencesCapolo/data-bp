...
import { GetPartidosNacionalChannelsUseCase } from '@partidos/core/use-cases/queries/GetPartidosNacionalChannelsUseCase';
...
export async function GET(req: NextRequest) {
  ...
  try {
    const dto = await new GetPartidosNacionalChannelsUseCase(composePartidosNacionalRepo())
      .execute(parsed.data);
    return ok(dto);
  } catch (err) {
