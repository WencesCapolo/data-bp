...
- ~~Auth on GET routes~~ → **done**. `src/proxy.ts` (Next 16 proxy convention — replaces deprecated middleware) checks the portal session cookie on `/`, `/basket/*`, `/partidos/*`, `/financiero/*` and their `/api/*`; redirects pages to the portal `/login?redirectTo=`, returns 401 for API. Every API route also gates itself with `getSessionUser()` (ADR 0008). `/api/basket/sync` allows `x-sync-token` bypass for cron.
...
- ~~Multi-dashboard landing + RBAC~~ → done. `/` lists dashboards (`src/lib/dashboards.ts` registry: `Suscripciones` live → `/basket`, `Financiero` soon). Roles `admin`/`viewer` derive from the `analytics` Acceso in the portal's Auth DB (ADR 0008): Nivel `admin` → admin, `read`/`write` → viewer. `requireSession` / `requireDashboard` in `src/lib/auth/rbac.ts`. The former allowlist, `/admin` UI and `/api/admin/users` are gone; Accesos are managed in the portal.
